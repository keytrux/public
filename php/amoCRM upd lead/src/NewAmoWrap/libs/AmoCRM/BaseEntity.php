...
use Roistat\AmoCRM_Wrap\Helpers\CustomField;
use Roistat\AmoCRM_Wrap\Helpers\Value;
use Exception;
use stdClass;
...
abstract class BaseEntity extends Base
{
    ...
    protected function load($id)
    {
        ...
        if ($id !== null) {
            $res = AmoCRM::cUrl("api/v4/$typeUrl/$id/links");
            $this->links = $res->_embedded->links;
        }

        $res = AmoCRM::cUrl($link);
        if ($res !== null && count($res->_embedded->leads) > 0) {
            $this->loadInRaw(current($res->_embedded->leads));
        } else if (count($res->_embedded->$typeUrl) > 0) {
            $this->loadInRaw(current($res->_embedded->$typeUrl));
        } else {
