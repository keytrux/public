let dateStart, dateEnd, listFilter, listResult, listStores, listOptions, selectedListStore, arrayStores = [], arrayOptions = ["Отгрузка", "Продажи по наличке", "Продажи по карте"], arrayResult = [], blocking = false, file = null;
...
function submitFunction(e) {
	...
	let params = "method=get&accountId=" + accountId + "&dateStart=" + dateStart[0].value + "&dateEnd=" + dateEnd[0].value + "&stores=" + JSON.stringify(arrayStores) + "&options=" + JSON.stringify(arrayOptions);
	ajax(params, function(data) {
		blocking = false;
		if (data.length == 0) return;
		let href, id;
		if (data[0]) {
			data[0].forEach(function(rows) {
				rows.forEach(function(row) {
					href = row['store']['meta']['href'];
					id = href.replace("https://api.moysklad.ru/api/remap/1.2/entity/store/", "");
					let index = arrayResult.findIndex(array => array.id == id);
					if (index != -1) {
		...
		if (data[3]) {
			data[3].forEach(function(rows) {
				let index = arrayResult.findIndex(array => array.id == rows[0]);
				if (index == -1) return;
				rows[1].forEach(function(row) {
					row.forEach(function(r) {
						arrayResult[index]["seb"] += r['sellCostSum'] - r['returnCostSum'];
					});
				});
			});
		}
